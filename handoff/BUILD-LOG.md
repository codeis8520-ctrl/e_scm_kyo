...
- 레이아웃 후속수정(사용자 피드백): 표가 max-w-2xl(672px) 좁은 컬럼을 넘쳐 어긋나고 fixed 하단바가 뷰포트 중앙정렬(사이드바 ml-64 미보정)이라 콘텐츠 대비 128px 어긋나며 행 가림. → ①컨테이너 `showFullGrid ? max-w-5xl : max-w-2xl`(표 모드만 확대) ②하단 버튼바 `fixed bottom-0 left-0 right-0 max-w-2xl mx-auto` → `sticky bottom-0`(콘텐츠 컬럼 정렬 + 흐름공간 차지로 행 안 가림, pb-28 제거). main에 overflow:hidden 없어(window 스크롤) sticky 정상.
- 판매가 컬럼 추가(사용자 피드백): 표 품목↔오픈재고 사이에 '판매가' 열 신설. l.unit_price(=getReportTemplate에서 products.price로 채움, 저장 스냅샷) 읽기전용 우측정렬 표시. 등록 판매가 참조용(편집 불가).
- 재고 연동 + 승인지연 장치(사용자 피드백, PO 결정 B=참조열+불일치경고): 오픈재고를 실재고로 직접 연동하려다 **승인 시점에만 inventories 갱신되는 timing 결함**(전일 미승인 시 오늘 오픈재고 stale) 발견 → PO가 (B) 선택. ①오픈재고=직전 일보 마감 이월 유지(승인 무관 연속, Phase1 원복) ②표에 '시스템재고' 열 추가=현재 inventories.quantity 라이브 표시(getReportTemplate inv quantity 맵 + getDailyReport 라이브 조회 주입, DailyReportLineInput.system_stock 표시용·미저장) ③오픈재고≠시스템재고면 ⚠(amber)+title(승인지연/외부변동 안내). schema.ts opening_stock 주석 동기화. 마이그 없음.
  대안 미채택: (A)자동보정=실재고−미승인이전일보 순감소, (C)경고배너만. PO가 B(연속 이월 + 라이브 대조) 선택.
