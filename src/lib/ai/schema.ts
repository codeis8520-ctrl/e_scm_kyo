...
inventory_movements: id, branch_id, product_id, movement_type(IN/OUT/ADJUST/TRANSFER/PRODUCTION), quantity(NUMERIC(14,4), 마이그 087·문자열 직렬화 주의), memo, created_at, usage_type_id(소모 사용유형 FK, reference_type=USAGE 일 때만 값 존재; 그 외 NULL), created_by(처리자 users FK, 마이그 095·NULL 허용; 재고 소모(USAGE) 등록 시 기록, 과거·자동생성분은 NULL)
  ...
  ※ **#92 created_at=전표 기준일자 원칙**: inventory_movements.created_at은 전표 생성시각이 아니라 **전표 기준일자**다. POS_SALE/PHANTOM_DECOMPOSE=판매전표 ordered_at(판매일자), ONLINE_SALE=자사몰 ordered_at, TRANSFER=이동일자. 과거일자 소급입력 시에도 이력·재고 날짜질의가 실제 업무일자와 일치. 단 SALE_CANCEL/CREDIT_CANCEL/USAGE/ADJUST/PURCHASE_RECEIPT/PRODUCTION_ORDER은 해당 사건 발생시점(현재 백데이팅 입력구 없음→now=업무일자). 전표 생성시각(내부 로그)은 부모 전표(sales_orders.created_at 등)에 보존.
  ※ **재고변동전표(통합, #85·#86 마이그106)**: stock_movement_docs(id, doc_no UNIQUE 'TR/US/AJ-YYYYMMDD-rand', move_type(TRANSFER/USAGE/ADJUST), from_branch_id=기준창고, to_branch_id(이동만; 그 외=from), usage_type_id(자가사용 헤더 기본 사유 FK→inventory_usage_types), ship_date·arrival_date(이동만), **movement_date(#107 마이그115: USAGE/ADJUST 업무 기준일자=사용자입력. 현황·이력·재고 기준. created_at은 내부 로그)**, memo, item_count, total_qty, created_by, created_at) + stock_movement_doc_items(doc_id FK, product_id, product_name·product_code 스냅샷, quantity(이동/사용=수량, 강제조정=목표값), **usage_type_id·reason(#107 마이그115: 자가소모 품목별 사유. 라인이 헤더보다 우선)**). 창고이동·자가사용·강제조정 각 1건=전표 1건. inventory_movements(실제 재고효과)와 별개의 전표 헤더(reference_id 연결 없음, 품목은 doc_items가 출처). **#107: USAGE/ADJUST movements.created_at도 movement_date(정오 KST)로 기록 → 현황·이력이 업무일자 기준.** 조회=getStockMovementDocs(유형/일자(created_at)/기준·대상 지점/**품목명** 필터, #94)·getStockMovementDocDetail. 재고관리>재고변동전표 입력/조회 탭. transferInventoryBatch·recordStockUsage·adjustInventoryBatch가 처리 후 createStockMovementDoc로 전표 동시 생성.
  ※ **#100 음수 수량 허용**: 자가사용(recordStockUsage)·POS판매(#84)는 음수 수량 허용 — 반품·정정·재고 복원. 음수 시 movement 부호 반전(USAGE 음수=IN 복원, OUT 아님), quantity=절대값, 재고 산술 자동 반대. 팬텀도 부호 유지 분해. 0만 차단. 창고이동(TRANSFER)은 양수만(반대는 반대전표 #94), 강제조정은 목표값이라 부호 무관. doc_items.quantity는 부호 그대로 스냅샷.
  ※ **재고변동전표 취소·반대전표·정정(#94 마이그109, #107)**: stock_movement_docs에 status('ACTIVE'/'REVERSED'=취소됨/'REVERSAL'=반대전표)·reversal_of(→원전표)·cancel_reason·cancelled_by/at. **#107 마스터 권한(SUPER_ADMIN·EXECUTIVE·HQ_OPERATOR)**: **reverseStockMovementDoc(docId, reason)** — 유형 무관(이동·사용·조정) 취소. 원전표의 실제 재고이동(reference_id=docId)을 그대로 반전(팬텀 분해·양지점까지 정확), 연결 없는 과거 전표는 유형별 폴백(TRANSFER=역이동, USAGE=음수 재소모, ADJUST=폴백 불가→새 조정 안내). 원전표 REVERSED+사유, 반대전표 REVERSAL+reversal_of. **updateStockMovementDocMeta(docId, {movement_date, memo, lines[]}, reason)** — 재고 불변 값(기준일자·메모·품목별 사유) in-place 정정, 연결 movements의 created_at·memo 동기화, doc_no 유지, audit. 품목·수량·창고 변경은 취소+재입력(정합 원칙). (구 reverseStockTransfer는 TRANSFER 전용 레거시, reverseStockMovementDoc로 대체.) 이미 REVERSED/REVERSAL은 재취소·정정 불가.
...
daily_sales_reports(마이그101+103, 판매일보 헤더): id, branch_id, report_date, author_user_id, author_name(스냅샷), status('DRAFT'|'SUBMITTED'|'APPROVED'), daily_total(현장매출+택배매출 합), note, approved_by/approved_at, posted(BOOL 멱등 플래그)/posted_at, journal_entry_id(승인 시 생성 매출분개) — UNIQUE(branch_id, report_date).
  ※ Phase1=기록 전용(DRAFT/SUBMITTED, 재고·회계 무관). Phase2a 승인(APPROVED)부터 재고·회계 반영. 백화점 매장 사원 휴대폰 종이일보 대체.
  ※ 🔴 승인(approveDailyReport, 관리자만): ①매출분개 먼저(createSaleJournal, totalAmount=현장+택배매출, **차변 미수금 1115**(credit·백화점 월정산), COGS=Σ(onsite_sold×cost, 팬텀 BOM 분해), 대변 매출4110/VAT2151) — 실패/대차불일치 시 전체 throw(재고 미적용) ②재고 movements(라인 onsite_sold+sample_damage→OUT / in_return→IN, reference_type='DAILY_REPORT' reference_id=report_id, 음수 허용·NUMERIC) ③헤더 posted=true 조건부 update(동시성 멱등). **시음/파손(sample_damage)=재고 OUT만, 분개 없음(GL 1130 갭 수용)**. 본사택배(hq_parcel)=재고/매출 무영향.
  ※ 🔴 승인취소(unpostDailyReport): ①역분개 먼저(createSaleJournal 음수 totalAmount + reversalOf=journal_entry_id, sourceType='DAILY_REPORT_CANCEL') ②반대 movements 신규 insert(reference_type='DAILY_REPORT_CANCEL', 원본 DAILY_REPORT movements 조회 후 OUT↔IN, 원본 삭제 금지·감사이력) ③posted=false·status=SUBMITTED 되돌림. 멱등(posted 조건부 + CANCEL movements 존재 가드).
  ※ 승인(posted=true) 후 saveDailyReport 차단(E3 수정잠금) — 승인취소 선행.
  ※ **조회·변경이력(#93 Phase1, 마이그108)**: listDailyReportsRange(from/to/branchId/status)=날짜범위·지점 브라우징. getDailyReportRevisions(reportId)=변경이력. **daily_sales_report_revisions**(id, report_id→CASCADE, branch_id, report_date, snapshot JSONB{header,lines}=변경 직전 상태, daily_total, edited_by/edited_by_name, change_note, created_at): 제출(SUBMITTED)된 일보 재수정 시 saveDailyReport가 직전 상태 1건 스냅샷. 일보 화면 관리자 '조회' 탭. (※ 일보는 여전히 sales_orders 미생성=독립 집계, #93 완전통일은 Phase2 후보.)
daily_sales_report_lines(마이그101, 판매일보 라인): id, report_id(→daily_sales_reports CASCADE), product_id(→products, NULL허용), product_code/product_name/unit_price(스냅샷), opening_stock(오픈=직전 일보 마감 이월, 승인 무관 연속. UI는 현재 inventories.quantity를 system_stock으로 라이브 대조해 불일치 시 ⚠ — 승인지연/외부변동 감지. system_stock은 표시용·미저장), onsite_sold(현장판매), sample_damage(시음증정/파손), in_return(입고/반품), closing_stock(마감=opening+in_return−onsite_sold−sample_damage 자동+사원수정 최종값), hq_parcel(본사택배 수량), onsite_revenue(현장매출), parcel_revenue(택배매출), sort_order — UNIQUE(report_id, product_id).
`;
...
전표 배송전환(방문↔택배, 수정가능 전표=COMPLETED·receipt_status∉{RECEIVED,null}만): 방문→택배는 미수령 품목을 PARCEL_PLANNED로 바꾸고 shipment(status=PENDING) 생성, 택배→방문은 shipment.status='PENDING'(송장 미발행)일 때만 shipment 삭제 후 품목 RECEIVED 전환. 금액 불변(배송비 없음). RECEIVED 품목은 보존.
  ※ **#97 수령방식 변경 ↔ 택배관리 실시간 연동**: updateSalesOrderDetails(판매상세 수령상태 드롭다운)가 receipt_status의 PARCEL_PLANNED 경계 넘김을 감지 — **택배→비택배(퀵/방문/수령완료): PENDING shipment 삭제(택배관리서 제외), 이미 발송(PRINTED+)이면 차단(경고)**; **비택배→택배: createPendingShipmentForOrder로 PENDING shipment 생성(택배관리 등록)**. 택배관리는 독립목록이 아니라 receipt_status=PARCEL_PLANNED 전표의 필터뷰. convertOrderToParcel/Pickup(전용 버튼)도 동일 연동.
...
- (참고) 수동 경로 잔존: 배송 카페24 주문탭 registerCafe24Customers(cafe24-actions)로도 고객 생성+연결 가능(검수 후 일괄). **#67: 내부적으로 autoRegisterOnlineCustomer 위임 → 자동수집과 100% 동일 dedup**(전화 단독, name 비파괴, 전화일치·이름불일치=needs_review). 자동 등록으로 대부분 커버되나 needs_review·미결제 건의 수동 보정 수단으로 유지. 반환에 review 카운트 포함.
...
- 배송 카페24 주문탭 "✓고객/확인필요/미등록" 판정(#67): 확정(배송추가)주문은 **실제 sales_orders.customer_id 연결 상태** 기준(미연결이면 미등록=등록대상, 등록 후 즉시 ✓). 미확정주문은 **휴대폰(digits) 단독 매칭**: 전화+이름 일치→✓고객(customer_match), 전화 일치·이름 불일치 또는 동일번호 다중→customer_review=true(🔶 확인 필요, 등록 시 기존 고객 연결+needs_review), 전화 무매칭→미등록. customers.phone 대시포맷(010-XXXX-XXXX) UNIQUE.
- webhook **미결제(handleOrderCreated)** 자동연결 기준 = linkOrCreateCustomer 이름 AND 전화(보수적): ①member_id ②이름AND전화, 생성 안 함. **결제완료(handleOrderPaid)·스마트스토어 import·수동 registerCafe24Customers(#67)** 는 모두 autoRegisterOnlineCustomer(전화 단독 dedup + 자동생성 + needs_review)로 일원화.
