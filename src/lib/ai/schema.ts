...
inventory_movements: id, branch_id, product_id, movement_type(IN/OUT/ADJUST/TRANSFER/PRODUCTION), quantity(NUMERIC(14,4), 마이그 087·문자열 직렬화 주의), memo, created_at, usage_type_id(소모 사용유형 FK, reference_type=USAGE 일 때만 값 존재; 그 외 NULL), created_by(처리자 users FK, 마이그 095·NULL 허용; 재고 소모(USAGE) 등록 시 기록, 과거·자동생성분은 NULL)
  ...
  ※ **재고변동전표(통합, #85·#86 마이그106)**: stock_movement_docs(id, doc_no UNIQUE 'TR/US/AJ-YYYYMMDD-rand', move_type(TRANSFER/USAGE/ADJUST), from_branch_id=기준창고, to_branch_id(이동만; 그 외=from), usage_type_id(자가사용 FK→inventory_usage_types), ship_date·arrival_date(이동만), memo, item_count, total_qty, created_by, created_at) + stock_movement_doc_items(doc_id FK, product_id, product_name·product_code 스냅샷, quantity(이동/사용=수량, 강제조정=목표값)). 창고이동·자가사용·강제조정 각 1건=전표 1건. inventory_movements(실제 재고효과)와 별개의 전표 헤더(reference_id 연결 없음, 품목은 doc_items가 출처). 조회=getStockMovementDocs(유형/일자(created_at)/기준·대상 지점/**품목명** 필터, #94)·getStockMovementDocDetail. 재고관리>재고변동전표 입력/조회 탭. transferInventoryBatch·recordStockUsage·adjustInventoryBatch가 처리 후 createStockMovementDoc로 전표 동시 생성.
  ...
  ※ **창고이동 취소·반대전표(#94 마이그109)**: stock_movement_docs에 status('ACTIVE'/'REVERSED'=취소됨/'REVERSAL'=반대전표)·reversal_of(→원전표)·cancel_reason·cancelled_by/at 추가. **reverseStockTransfer(docId, reason)**(본사 권한만, move_type=TRANSFER만): 도착→출발 역방향으로 transferInventoryBatch 재실행=반대전표 생성(재고 반대 반영, 이력=새 inventory_movements), 원전표 status=REVERSED+사유, 반대전표 status=REVERSAL+reversal_of=원전표. 직접 수정 대신 반대전표로 되돌림(재고 정합). 이미 REVERSED/REVERSAL은 재취소 불가. 도착지 재고 부족 시 실패(되돌릴 재고 없음).
