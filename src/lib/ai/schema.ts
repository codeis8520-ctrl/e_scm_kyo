...
inventory_movements: id, branch_id, product_id, movement_type(IN/OUT/ADJUST/TRANSFER/PRODUCTION), quantity(NUMERIC(14,4), 마이그 087·문자열 직렬화 주의), memo, created_at, usage_type_id(소모 사용유형 FK, reference_type=USAGE 일 때만 값 존재; 그 외 NULL), created_by(처리자 users FK, 마이그 095·NULL 허용; 재고 소모(USAGE) 등록 시 기록, 과거·자동생성분은 NULL)
  ...
  ※ **지점이동 전표(#85 마이그106)**: stock_transfers(id, transfer_no UNIQUE 'TR-YYYYMMDD-rand', from_branch_id, to_branch_id, ship_date, arrival_date, memo, item_count, total_qty, created_by, created_at) + stock_transfer_items(transfer_id FK, product_id, product_name·product_code 스냅샷, quantity). 창고이동 1건=전표 1건. 위 TRANSFER 무브먼트는 reference_id=stock_transfers.id 로 연결. 조회=getStockTransfers(일자/출발/도착 필터)·getStockTransferDetail. 재고관리>지점이동 조회 탭.
