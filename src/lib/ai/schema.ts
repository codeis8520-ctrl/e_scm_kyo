...
sales_orders.receipt_status: 제품 수령 흐름 (현장판매는 대부분 RECEIVED).
  ...
  ※ 배송→수령 자동연동(#43): 배송목록/AI/카페24웹훅에서 shipment SHIPPED는 수령상태 불변(발송은 shipment.status로만 추적), DELIVERED 시 택배품목→RECEIVED(+receipt_date). 방문/퀵/이미수령 품목 무손상. 공용 헬퍼 syncReceiptStatusFromShipment(receipt-sync.ts).
