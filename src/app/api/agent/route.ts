...
import { claudeClient, AnthropicMessage } from '@/lib/ai/client';
...
9. 지원하지 않는 기능을 요청받으면: "해당 기능은 현재 지원하지 않습니다." 한 줄로만 답한다.
10. 응답은 반드시 완결된 문장으로 끝내야 한다.
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── Agentic loop (최대 6회) ───────────────────────────────────────────────
    for (let i = 0; i < 6; i++) {
      const { content, stop_reason } = await claudeClient.chat(systemContent, messages, AGENT_TOOLS);
      ...
      if (stop_reason !== 'tool_use') {
        ...
        return NextResponse.json({
          type: 'success',
          message: (text as any)?.text || '처리 완료',
        });
      }
      ...
      if (writeBlock) {
        const description = buildConfirmDescription(writeBlock.name, writeBlock.input);
        return NextResponse.json({
          type: 'confirm',
          message: description,
          pending_action: { tool: writeBlock.name, args: writeBlock.input, description },
        });
      }

      // 읽기 도구 실행 → tool_result 메시지 추가
      const toolResults: any[] = [];
      for (const block of toolUseBlocks) {
        const result = await executeTool(block.name, block.input, supabase);
        extractMemory(db, block.name, block.input, result).catch(() => {});
        toolResults.push({
          type: 'tool_result',
...
function buildSuccessDetail(tool: string, parsed: any): string {
  switch (tool) {
    ...
    case 'bulk_update_product_costs': {
      const pct = Math.round((parsed.cost_ratio || 0) * 100);
...
function buildConfirmDescription(toolName: string, args: Record<string, any>): string {
  ...
  switch (toolName) {
    ...
    case 'receive_purchase_order':
      lines.push('📥 입고 처리 확인');
      add('발주번호', args.order_number); add('메모', args.memo);
...
export async function GET() {
  return NextResponse.json({ status: 'ok', model: claudeClient.model, tools: AGENT_TOOLS.length });
}
