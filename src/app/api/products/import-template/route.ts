...
export async function GET() {
  ...
  const example1 = [
    ...
    '과세',
    '예',
    '제품 / 더경옥 제품 / 단지',  // 카테고리: 코드 관리 화면의 경로명
    '대표 완제품 예시',
  ];
  const example2 = [
    '홍삼정',
  ...
  const guide = [
    ...
    ['2. 유형: "완제품" / "원자재" / "부자재" / "무형상품" 중 하나. 비우면 "완제품".'],
    ['   ※ 제품 등록 화면의 4가지 유형 라벨과 동일하게 입력하세요.'],
    ['3. 부가세: "과세" 또는 "면세". 비우면 "과세". 면세 품목은 한약류·식품류 등.'],
    ['4. 재고관리: "예" 또는 "아니오". 비우면 무형상품은 "아니오", 그 외는 "예".'],
    ['5. 바코드는 완제품 유형만 저장됩니다 — 원자재·부자재·무형상품은 입력해도 무시됩니다.'],
    ['6. 판매가/원가는 숫자(원). 콤마는 자동 제거됨.'],
    ['7. 카테고리는 시스템 코드 → 카테고리 탭에서 등록한 항목을 다음 중 하나 형식으로:'],
    ['   • 전체 경로명 — "제품 / 더경옥 제품 / 단지"'],
    ['   • 위치 코드 — "[1-1-1]" 또는 "1-1-1"'],
    ['   • 잎 이름 — "단지" (중복 시 첫 번째 매칭). 모호하면 경로명 권장.'],
    ['   ※ 카테고리 탭에 없는 이름은 매칭되지 않고 카테고리가 비워집니다.'],
    ['8. 동일 코드가 이미 등록돼 있으면 빈 칸이 아닌 항목만 업데이트됩니다.'],
    ['9. 신규 등록 시 재고관리="예"이면 모든 활성 지점에 재고 0 레코드 자동 생성.'],
