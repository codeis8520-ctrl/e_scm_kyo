...
export default function NotificationTemplateClassifier() {
  ...
  const setField = (tplId: string, field: keyof RowEdit, value: any) => {
    const cur = getCurrent(tplId);
    setLocalEdits(prev => ({ ...prev, [tplId]: { ...cur, [field]: value } }));
  };
    ...
    <div className="space-y-4">
      <div className="card">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1 leading-relaxed">
            ...
            수동 발송 가능으로 지정된 템플릿만 <Link href="/notifications" className="text-blue-600 underline">알림톡 발송</Link> 화면에 기본 노출됩니다.
          </p>
        </div>
        ...
        {unclassified > 0 && (
          <div className="mt-3 p-2.5 rounded-lg bg-amber-50 border border-amber-200 text-xs text-amber-700">
            ⚠️ <b>미분류 {unclassified}개</b> — 분류하기 전에는 알림톡 발송 화면에서 표시되지 않을 수 있습니다.
          </div>
        )}
      </div>
      ...
      {loading ? (
        ...
        orderedKeys.map(eventKey => {
          ...
          const hint = isUnclassifiedGroup ? null : EVENT_TRIGGER_HINTS[eventKey];
